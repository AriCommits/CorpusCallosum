    ...
    for char in dangerous_chars:
        if char in editor:
            raise CommandInjectionError(
                f"Potentially dangerous character '{char}' in editor command: {editor}"
            )
...
def safe_subprocess_run(
    ...
    if shell and isinstance(command, str):
        raise SecurityError(
            "Using shell=True with string commands is dangerous. Use a list of arguments instead."
        )
    ...
    return subprocess.run(command, shell=shell, timeout=timeout, **kwargs)
